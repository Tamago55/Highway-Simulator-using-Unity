...
namespace TrafficSimulation {
    ...
    */
    ...
    public class VehicleAI : MonoBehaviour
    {
        ...
        [Tooltip("The maximum angle of the slope that the vehicle can climb")]
        public float maxClimbAngle = 60f;
        [Tooltip("The force applied to the vehicle when climbing a slope")]
        ...
        void MoveVehicle(){
            ...
            if(vehicleStatus == Status.STOP){
                ...
                // Check slope angle and apply additional force if necessary
        ...
        // This is changing point for adapting car simulation
        int GetNextSegmentId(){
            var nextSegments = trafficSystem.segments[currentTarget.segment].nextSegments;
            if(nextSegments.Count == 0)
                return 0;

            // If there are 3 segments, choose a segment randomly
            if(nextSegments.Count == 3) {
                float randomValue = Random.value; // this will return a float between 0 and 1

                if(randomValue < 0.8f)
                {
                    // 80% chance to return the smallest segment id
                    return nextSegments[0].id;
                }
                else
                {
                    // 20% chance to return the second smallest segment id
                    return nextSegments[1].id;
                }
            } 
            else 
            {
                ...
                // If there's an obstacle, choose the right segment
                if(obstacle != null){
                    // Assuming the next segment on the right is the segment with larger id
                    Segment largestSegment = nextSegments[0];
                    foreach (var segment in nextSegments)
                    {
                        if(segment.id > largestSegment.id)
                        {
                            largestSegment = segment;
                        }
                    }

                    return largestSegment.id;
                } else {
                    // If there are less than 3 segments, or there's no obstacle, choose the smallest segment
                    Segment smallestSegment = nextSegments[0];
                    foreach (var segment in nextSegments)
                    {
                        if(segment.id < smallestSegment.id)
                        {
                            smallestSegment = segment;
                        }
                    }

                    return smallestSegment.id;
                }
            }
        }
        ////////////////////////////////////////////////////////////////////////
